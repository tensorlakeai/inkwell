...
from tensordoc.components.utils import (
...
class NotSupportedShapeError(Exception):
    ...
    it will raise NotSupportedShapeError. It is expected to be fixed in the future versions.
    See
...
class InvalidShapeError(Exception):
    """For shape operations like intersection of union, it will raise the InvalidShapeError when
...
class Interval(BaseCoordElement):
    ...
    def __init__(
        self, start, end, axis, canvas_height=None, canvas_width=None
    ):

        assert (
            start <= end
        ), f"Invalid input for start and end. Start must <= end."
        self.start = start
        self.end = end
        ...
        assert axis in [
            "x",
            "y",
        ], f"Invalid axis {axis}. Axis must be in 'x' or 'y'"
        self.axis = axis
    ...
    def relative_to(self, other):
        ...
        if isinstance(other, Interval):
            ...
            return (
                self.put_on_canvas(other).to_quadrilateral().relative_to(other)
            )
    ...
    def intersect(self, other: BaseCoordElement, strict: bool = True):
        ...
        if isinstance(other, Interval):
            ...
            if self.axis == "x":
                return Rectangle(
                    max(x_1, self.start), y_1, min(x_2, self.end), y_2
                )
            elif self.axis == "y":
                return Rectangle(
                    x_1, max(y_1, self.start), x_2, min(y_2, self.end)
                )
    ...
    def union(self, other: BaseCoordElement, strict: bool = True):
        ...
        if isinstance(other, Interval):
            ...
            if self.axis == "x":
                return Rectangle(
                    min(x_1, self.start), y_1, max(x_2, self.end), y_2
                )
            elif self.axis == "y":
                return Rectangle(
                    x_1, min(y_1, self.start), x_2, max(y_2, self.end)
                )
    ...
    def is_in(self, other, soft_margin={}, center=False):
        ...
        if isinstance(other, Interval):
            ...
            if not center:
                ...
                is_vertice_in = [
                    vertice_in_polygon(vertice, other.points)
...
class Quadrilateral(BaseCoordElement):
    ...
    def __init__(
        self,
        points: Union[np.ndarray, List, List[List]],
        height=None,
        width=None,
    ):
    ...
    def intersect(self, other: BaseCoordElement, strict: bool = True):
        ...
        if strict:
            raise NotSupportedShapeError(
                "The intersection between a Quadrilateral and other objects might generate Polygon shapes that are not supported. You can pass `strict=False` in the input that converts the Quadrilateral to Rectangle to avoid this Exception."
            )
        else:
