...
from tensordoc.components.base import BaseLayoutElement
...
class Layout(MutableSequence):
    ...
    def __init__(
        self, blocks: Optional[List] = None, *, page_data: Dict = None
    ):

        if not (
            (blocks is None)
            or (
                isinstance(blocks, Iterable)
                and blocks.__class__.__name__ != "Layout"
            )
        ):

            if blocks.__class__.__name__ == "Layout":
                error_msg = f"Please check the input: it should be Layout([layout]) instead of Layout(layout)"
            else:
        ...
        if isinstance(blocks, tuple):
            blocks = list(
                blocks
            )  # <- more robust handling for tuple-like inputs
    ...
    def __add__(self, other):
        if isinstance(other, Layout):
            ...
            return self.__class__(
                self._blocks + other, page_data=self.page_data
            )
        else:
            raise ValueError(
    ...
    def sort(
        self, key=None, reverse=False, inplace=False
    ) -> Optional["Layout"]:
        ...
        if not inplace:
            return self.__class__(
                sorted(self._blocks, key=key, reverse=reverse),
                page_data=self.page_data,
            )
        else:
    ...
    def shift(self, shift_distance):
        ...
        """
        return self.__class__(
            [ele.shift(shift_distance) for ele in self],
            page_data=self.page_data,
        )
    ...
    def get_info(self, attr_name):
        ...
        """
        return [
            getattr(ele, attr_name) for ele in self if hasattr(ele, attr_name)
        ]
            ...
            for ele in self:
                if isinstance(ele, TextBlock):
                    ...
                    if ele.block._name != target_coord_name:
                        ele.block = getattr(
                            ele.block, f"to_{target_coord_name}"
                        )()
                else:
